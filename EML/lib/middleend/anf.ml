...
let rec anf (expr : expr) (k : immediate -> anf_expr t) : anf_expr t =
  ...
  | ExpLet (flag, (pat, expr), _, body) ->
    (match pat with
     | PatAny | PatUnit -> anf expr (fun _ -> anf body k)
     | PatTuple (p1, p2, rest) ->
  ...
  | ExpFunction (exp1, exp2) ->
    let func, args_list =
      let rec collect_args acc = function
        | ExpFunction (f, arg) -> collect_args (arg :: acc) f
        | f -> f, acc
      in
      collect_args [] (ExpFunction (exp1, exp2))
    in
  ...
  | ExpLambda (params, body) ->
    let* body_anf_expr =
      anf body (fun imm -> return (AnfExpr (ComplexImmediate imm)))
    in
    ...
    (match lambda_anf with
     | AnfExpr (ComplexLambda (pats, body)) -> bind_complex_expr (ComplexLambda (pats, body)) k
     | _ -> fail "ExpLambda: wrap_params must return ComplexLambda")
  | ExpUnit -> bind_complex_expr ComplexUnit k
