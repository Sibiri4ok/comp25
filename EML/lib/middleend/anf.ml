...
let optimize_anf_let rf name1 v body = (* Optimize  let x = y in x -> y*)
  match rf, body with
  | NonRec, AnfExpr (ComplexImmediate (ImmediateVar name2)) when String.equal name1 name2 -> AnfExpr v
...
let rec anf (e : expr) (k : immediate -> anf_expr t) : anf_expr t =
  ...
  | ExpBinOper (op, e1, e2) ->
      anf e1 (fun immediate1 ->
        anf e2 (fun immediate2 ->
          let* var_name = fresh in
          let* cont_expr = k (ImmediateVar var_name) in
          return (AnfLet (NonRec, var_name, ComplexBinOper (op, immediate1, immediate2), cont_expr))))
  ...
  | ExpBranch (cond, then_expr, else_expr) ->
      anf cond (fun immediate_cond ->
        let* then_anf = anf then_expr (fun immediate -> return (AnfExpr (ComplexImmediate immediate))) in
        let* var_name = fresh in 
        let* cont_expr = k (ImmediateVar var_name) in 
        let* else_anf =
        ...
        in return (AnfLet (NonRec, var_name, ComplexBranch (immediate_cond, then_anf, else_anf), cont_expr)))
  
  | ExpLet (rec_flag, (pat, e1), _, e2) ->
      let* e1_anf = anf e1 (fun immediate -> return (AnfExpr (ComplexImmediate immediate))) in
      ...
      if is_simple_pattern pat then
        (match pattern_to_ident pat with
         | Some name ->
             return (AnfLet (rec_flag, name, complex_expr_body, e2_anf))
         | None ->
             let* var_name = fresh in
  ...
  | ExpLambda (patterns, body) ->
      let* body_anf = anf body (fun immediate -> return (AnfExpr (ComplexImmediate immediate))) in
      let* var_name = fresh in
      let* cont_expr = k (ImmediateVar var_name) in
  ...
  | ExpFunction (func, arg) ->
      anf func (fun immediate_func ->
        anf arg (fun immediate_arg ->
          let* var_name = fresh in
          let* cont_expr = k (ImmediateVar var_name) in
          return (AnfLet (NonRec, var_name, ComplexApp (immediate_func, immediate_arg, []), cont_expr))))
...
let anf_structure_item (item : structure) : anf_structure t =
  match item with
  | SEval expr ->
      let* result = anf expr (fun immediate -> return (AnfExpr (ComplexImmediate immediate))) in
      return (AnfEval result)
  
  | SValue (rec_flag, (pat, expr), _) ->
      if is_simple_pattern pat then
        let* anf_expr_body = anf expr (fun immediate -> return (AnfExpr (ComplexImmediate immediate))) in
        match pattern_to_ident pat with
        | Some name ->
            return (AnfValue (rec_flag, (name, anf_expr_body), []))
        | None ->
            return (AnfValue (rec_flag, ("_", anf_expr_body), []))
      else
